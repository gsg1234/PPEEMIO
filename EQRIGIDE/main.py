...
if __name__ == '__main__':
    ...
    for i in range(p_cuarto):
        q1, q3, seed = calculer_GD(tA_tray[i], th3, seed)
        q1A1.append(q1); q3A1.append(q3)
    for i in range(p_cuarto):
        q1, q3, seed = calculer_GD(tA_tray[p_cuarto-1-i], th3, seed)
        q1A1.append(q1); q3A1.append(q3)
        
    print('Calcul Étape 2 : th3 en mouvement...')
    q1A2, q3A2 = [], []
    for i in range(p_cuarto):
        q1, q3, seed = calculer_GD(th1, tB_tray[i], seed)
        q1A2.append(q1); q3A2.append(q3)
    for i in range(p_cuarto):
        q1, q3, seed = calculer_GD(th1, tB_tray[p_cuarto-1-i], seed)
        q1A2.append(q1); q3A2.append(q3)
        
    print('Calcul Étape 3 : th1 et th3 en mouvement...')
    q1A3, q3A3 = [], []
    for i in range(p_cuarto):
        q1, q3, seed = calculer_GD(tA_tray[i], tB_tray[i], seed)
        q1A3.append(q1); q3A3.append(q3)
    for i in range(p_cuarto):
        q1, q3, seed = calculer_GD(tA_tray[p_cuarto-1-i], tB_tray[p_cuarto-1-i], seed)
        q1A3.append(q1); q3A3.append(q3)

    Q1_total = np.vstack((q1A1, q1A2, q1A3))
    Q3_total = np.vstack((q3A1, q3A2, q3A3))
    ...
    Q1_GI, Q3_GI = [], []
    for i in range(p_cuarto):
        q1, q3, seed = calculer_GI(x_tray[i], -100, seed)
        Q1_GI.append(q1); Q3_GI.append(q3)
    for i in range(p_cuarto):
        q1, q3, seed = calculer_GI(20, y_tray[i], seed)
        Q1_GI.append(q1); Q3_GI.append(q3)
    for i in range(p_cuarto):
        q1, q3, seed = calculer_GI(x_tray[p_cuarto-1-i], -140, seed)
        Q1_GI.append(q1); Q3_GI.append(q3)
    for i in range(p_cuarto):
        q1, q3, seed = calculer_GI(-20, y_tray[p_cuarto-1-i], seed)
        Q1_GI.append(q1); Q3_GI.append(q3)
    animate(Q1_GI, Q3_GI, 1)
